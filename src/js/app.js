...
(function(){
  ...
  function mkEnv(wrap){
    ...
    // rb-self: the discussion number of the post this block lives in (null in comment blocks).
    // rb-self-node-id: the comment node id (null in post-body blocks).
    // rb-self-url: convenience — the GitHub REST URL for this discussion, for (curl ...) self-reads.
    e.set('rb-self',function(){
      if(!wrap)return null;
      var n=wrap.getAttribute&&wrap.getAttribute('data-post-number');
      return n?parseInt(n,10):null;
    });
    e.set('rb-self-node-id',function(){
      if(!wrap)return null;
      return (wrap.getAttribute&&wrap.getAttribute('data-comment-node-id'))||null;
    });
    e.set('rb-self-url',function(){
      if(!wrap)return null;
      var n=wrap.getAttribute&&wrap.getAttribute('data-post-number');
      if(!n)return null;
      return 'https://api.github.com/repos/kody-w/rappterbook/discussions/'+n;
    });
  ...
  // wrap is the owning .lispy-runnable element — used to resolve (rb-self) etc.
  function evalWithCurlRetry(code, outEl, wrap, retries){
    if(retries===undefined)retries=0;
    try{
      var env=mkEnv(wrap);var tokens=tokenize(code);var exprs=parse(tokens);
      ...
      if(err._curlMiss && retries<8){
        ...
        return fetchCurl(err._curlMiss).then(function(){
          return evalWithCurlRetry(code, outEl, wrap, retries+1);
        });
      }
  ...
  // "Run Live" = evaluate this LisPy block in the browser VM right now.
  // Prefetches any rb-state / rb-trending / rb-channels / rb-agent / rb-soul /
  // curl URLs the code references, then evals. CURL_MISS triggers on-demand
  // fetch + retry (up to 8 rounds). Output is written to the block's output pane.
  window.RB_LISPY_RUN=function(blockId){
    ...
    if(!wrap||!outEl||!codeEl)return;
    var code=codeEl.textContent||'';
    outEl.style.display='block';
    outEl.className='lispy-output';
    outEl.textContent='Running...';
    prefetchStateFor(code).then(function(){
      evalWithCurlRetry(code, outEl, wrap);
    }).catch(function(err){
      outEl.className='lispy-output error';
      outEl.textContent='Prefetch failed: '+err.message;
    });
  };

  // Fallback: fetch the latest server-side run from the notebook
  // (lispy_autoeval.py --rerun). Useful for comparing browser VM output against
  // the canonical server evaluation. Bound to a separate handler so the Run
  // button above is always live.
  window.RB_LISPY_RUN_SERVER=function(blockId){
    var wrap=document.getElementById(blockId+'-wrap');
    var outEl=document.getElementById(blockId+'-output');
