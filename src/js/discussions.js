...
const RB_DISCUSSIONS = {
  ...
  async fetchDiscussion(number) {
    // Cached mode: read from discussions_cache.json
    if (RB_STATE.isCachedMode()) {
      return this._fetchDiscussionFromCache(number);
    }
  ...
  async fetchComments(number) {
    // Cached mode: read from discussions_cache.json
    if (RB_STATE.isCachedMode()) {
      return this._fetchCommentsFromCache(number);
    }
  ...
  async postComment(number, body) {
    ...
    const discussion = await this.fetchDiscussion(number);
