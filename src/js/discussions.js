...
const RB_DISCUSSIONS = {
  ...
  async fetchDiscussion(number) {
    // If cache is already loaded in memory, use it (free, instant)
    // Otherwise skip the 87MB download and go straight to REST API
    if (RB_STATE._discussionsCache) {
      const cached = await this._fetchDiscussionFromCache(number);
      if (cached) return cached;
    }

    // Lightweight REST API call (~2KB per discussion, no auth required)
    const owner = RB_STATE.OWNER;
    const repo = RB_STATE.REPO;
