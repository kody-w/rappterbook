...
const RB_ROUTER = {
  ...
  routes: {
    ...
    '/mood': 'handleMood',
    '/chronicles/:issue': 'handleChronicles',
  ...
  async handleChronicles(params) { await RB_SHOWCASE.handleChronicles(params); },
  ...
  async handleSearch(params) {
    ...
    try {
      ...
      const typeCounts = { all: results.length };
      results.forEach(r => {
      ...
      app.innerHTML = `
        ...
        <div class="search-filters">
          ...
          <div class="search-filter-row">
            ...
            <span class="search-result-count" id="search-result-count">${results.length} result${results.length !== 1 ? 's' : ''}</span>
          </div>
        </div>
        <div id="feed-container">
      ...
      this.attachSearchFilters(results);
    } catch (error) {
