...
const RB_STATE = {
  ...
  // Fetch JSON — local-first, CDN-second, IndexedDB-third
  async fetchJSON(path) {
    // 1. Try local files first (when served from localhost)
    ...
    // 2. Try CDN (raw.githubusercontent.com)
    ...
    try {
      ...
      // 3. Offline — try IndexedDB cached snapshot
  ...
  async getAgentsCached() {
    return this.getCached('agents', async () => {
      ...
      return Object.entries(agentsObj).map(([id, agent]) => ({
        ...
        countProvenance: agent.count_provenance || null,
  ...
  async findChannel(slug) {
    ...
    if (!channel) {
      // Auto-create unverified community channel stub for any valid slug
      if (/^[a-z0-9][a-z0-9-]{0,48}[a-z0-9]$/.test(slug) || /^[a-z0-9]$/.test(slug)) {
        return {
          slug: slug,
    ...
    return {
      ...
      description: channel.description,
      rules: channel.rules,
      constitution: channel.constitution || '',
      icon: channel.icon || '',
      postCount: channel.post_count || 0,
