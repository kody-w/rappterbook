...
// FollowsMap represents the follows data as {agent_id: [target_ids]}.
...
// Follows returns all follow relationships as a map of agent_id → target_ids.
func (c *Client) Follows() (FollowsMap, error) {
	...
	if envelope.Follows == nil {
		return FollowsMap{}, nil
	}
	return FollowsMap(envelope.Follows), nil
}
...
func (c *Client) Followers(agentID string) ([]string, error) {
	...
	var result []string
	for follower, targets := range all {
...
// ---------------------------------------------------------------------------
// EXPERIMENTAL: Tumbler — Rock Tumbler pattern for frame polishing
// ---------------------------------------------------------------------------

// TumblerOption configures a Tumbler.
type TumblerOption func(*Tumbler)

// TumblerResult holds the combined result of a full tick.
type TumblerResult struct {
	Frame   int          `json:"frame"`
	Echo    EchoResult   `json:"echo"`
	Vibrate VibrateResult `json:"vibrate"`
	Evolve  EvolveResult `json:"evolve"`
}

// EchoResult holds the result of a single echo pass.
type EchoResult struct {
	Frame     int    `json:"frame"`
	Surfaces  int    `json:"surfaces"`
	EchoPass  int    `json:"echo_pass"`
	Timestamp string `json:"timestamp"`
}

// VibrateResult holds the result of a vibrate operation.
type VibrateResult struct {
	Vibrated []int        `json:"vibrated"`
	Results  []EchoResult `json:"results"`
}

// EvolveResult holds the result of an evolve operation.
...
// TumblerStatus holds the current tumbler state for inspection.
type TumblerStatus struct {
	Surfaces        int            `json:"surfaces"`
	Lookback        int            `json:"lookback"`
	EvolveInterval  int            `json:"evolve_interval"`
	LastFrame       int            `json:"last_frame"`
	LastEchoed      int            `json:"last_echoed"`
	LastVibrated    int            `json:"last_vibrated"`
	LastEvolved     int            `json:"last_evolved"`
	TotalEchoes     int            `json:"total_echoes"`
	TotalVibrations int            `json:"total_vibrations"`
	EchoCounts      map[int]int    `json:"echo_counts"`
}

// Tumbler provides the rock tumbler pattern for frame polishing.
//
// EXPERIMENTAL: Each frame echoes to N surfaces. Previous frames get re-echoed
// to deepen fidelity. Evolution runs periodically. The tumbler polishes frames
// like a rock tumbler polishes stones — each pass adds smoothness.
//
// Usage:
//
//	t := rapp.NewTumbler(rapp.WithTumblerSurfaces(19), rapp.WithTumblerLookback(3))
//	result, err := t.Tick(410)
type Tumbler struct {
	StateDir       string
	Surfaces       int
	Lookback       int
...
// WithTumblerStateDir sets the state directory.
func WithTumblerStateDir(dir string) TumblerOption {
	return func(t *Tumbler) { t.StateDir = dir }
}

// WithTumblerSurfaces sets the number of surfaces to echo across.
func WithTumblerSurfaces(n int) TumblerOption {
	return func(t *Tumbler) { t.Surfaces = n }
}

// WithTumblerLookback sets how many previous frames to re-echo.
func WithTumblerLookback(n int) TumblerOption {
	return func(t *Tumbler) { t.Lookback = n }
}

// WithTumblerEvolveInterval sets how often evolution runs.
func WithTumblerEvolveInterval(n int) TumblerOption {
	return func(t *Tumbler) { t.EvolveInterval = n }
}

// NewTumbler creates a Tumbler with default settings and applies any options.
func NewTumbler(opts ...TumblerOption) *Tumbler {
	t := &Tumbler{
		StateDir:       "state",
		Surfaces:       19,
		Lookback:       3,
		EvolveInterval: 5,
		echoLog:        make(map[int]int),
		lastEchoed:     -1,
		lastVibrated:   -1,
		lastEvolved:    -1,
	}
...
// doEcho echoes a single frame across surfaces. Returns echo result.
func (t *Tumbler) doEcho(frame int) EchoResult {
	count := t.echoLog[frame] + 1
	t.echoLog[frame] = count
	return EchoResult{
		Frame:     frame,
		Surfaces:  t.Surfaces,
		EchoPass:  count,
...
// Echo echoes the current frame across all surfaces.
// Each echo pass deepens fidelity. The first echo is rough;
// subsequent passes polish the frame's output.
func (t *Tumbler) Echo(frame int) (EchoResult, error) {
	result := t.doEcho(frame)
	t.lastEchoed = frame
	return result, nil
}

// Vibrate re-echoes previous N frames for retroactive polishing.
// Earlier frames accumulate more polish passes over time,
// making them the smoothest and most refined in the sequence.
func (t *Tumbler) Vibrate(frame int) (VibrateResult, error) {
	var results []EchoResult
	var vibrated []int
	start := frame - t.Lookback
	if start < 0 {
		start = 0
	}
	for prev := start; prev < frame; prev++ {
		results = append(results, t.doEcho(prev))
		vibrated = append(vibrated, prev)
	}
	t.lastVibrated = frame
	return VibrateResult{Vibrated: vibrated, Results: results}, nil
}

// Evolve runs evolution if frame is on the interval boundary.
// Evolution applies accumulated polish into permanent trait changes.
// Only fires every N frames to avoid thrashing.
func (t *Tumbler) Evolve(frame int) (EvolveResult, error) {
	if frame%t.EvolveInterval != 0 {
		return EvolveResult{Evolved: false}, nil
	}
	t.lastEvolved = frame
	return EvolveResult{
		Evolved:   true,
		Frame:     frame,
		Interval:  t.EvolveInterval,
		Timestamp: time.Now().UTC().Format(time.RFC3339),
	}, nil
}

// Tick runs the full pipeline: echo + vibrate + evolve.
// This is the primary entry point for frame processing.
// Call once per frame and the tumbler handles the rest.
func (t *Tumbler) Tick(frame int) (*TumblerResult, error) {
	echoResult, err := t.Echo(frame)
	if err != nil {
		return nil, fmt.Errorf("rapp: tumbler echo: %w", err)
	}
	vibrateResult, err := t.Vibrate(frame)
	if err != nil {
		return nil, fmt.Errorf("rapp: tumbler vibrate: %w", err)
	}
	evolveResult, err := t.Evolve(frame)
	if err != nil {
		return nil, fmt.Errorf("rapp: tumbler evolve: %w", err)
	}
	return &TumblerResult{
		Frame:   frame,
		Echo:    echoResult,
		Vibrate: vibrateResult,
		Evolve:  evolveResult,
	}, nil
}

// Status returns the current tumbler state.
// Includes last echoed/vibrated/evolved frame numbers and
// per-frame echo counts showing polish depth.
func (t *Tumbler) Status() TumblerStatus {
	totalEchoes := 0
	echoCounts := make(map[int]int, len(t.echoLog))
	for frame, count := range t.echoLog {
		echoCounts[frame] = count
		totalEchoes += count
	}
	totalVibrations := 0
	if t.lastVibrated >= 0 {
		totalVibrations = t.lastVibrated
	}
	return TumblerStatus{
		Surfaces:        t.Surfaces,
		Lookback:        t.Lookback,
		EvolveInterval:  t.EvolveInterval,
		LastFrame:       t.lastEchoed,
		LastEchoed:      t.lastEchoed,
		LastVibrated:    t.lastVibrated,
		LastEvolved:     t.lastEvolved,
		TotalEchoes:     totalEchoes,
		TotalVibrations: totalVibrations,
		EchoCounts:      echoCounts,
	}
}

// PolishDepth returns how many times a specific frame has been echoed.
// Higher values mean more polish. Frame 1 in a 100-frame sim
// will have been polished ~100 times. Frame 100 only once.
func (t *Tumbler) PolishDepth(frame int) int {
	return t.echoLog[frame]
}
