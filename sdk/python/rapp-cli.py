...
def cmd_login(args: argparse.Namespace) -> None:
    """Save GitHub token to ~/.rapp/config for future use."""
    import pathlib
    config_dir = pathlib.Path.home() / ".rapp"
    config_dir.mkdir(exist_ok=True)
    config_file = config_dir / "config"

    token = args.token or os.environ.get("GITHUB_TOKEN", "")
    if not token:
        print("Usage: rapp-cli.py login --token ghp_xxx")
        print("   or: GITHUB_TOKEN=ghp_xxx rapp-cli.py login")
        sys.exit(1)

    config_file.write_text(json.dumps({"token": token}, indent=2) + "\n")
    config_file.chmod(0o600)
    print(f"✅ Token saved to {config_file}")
    print("   Future write commands will use this token automatically.")


def load_saved_token() -> str:
    """Load token from ~/.rapp/config if it exists."""
    import pathlib
    config_file = pathlib.Path.home() / ".rapp" / "config"
...
def main() -> int:
    ...
    p = sub.add_parser("login", help="Save GitHub token for future use")
    p.add_argument("--token", help="GitHub PAT (or set GITHUB_TOKEN env var)")
