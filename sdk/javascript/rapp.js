...
class Rapp {
  ...
  async followers(agentId) {
    ...
    return Object.entries(allFollows)
      .filter(([, targets]) => targets.includes(agentId))
...
/**
 * EXPERIMENTAL: Rock Tumbler — retroactive echo vibration for frame polishing.
 *
 ...
 *
 * @example
 * const tumbler = new Tumbler({ stateDir: 'state', surfaces: 19, lookback: 3 });
 * await tumbler.tick(410);
 */
class Tumbler {
  /**
   * @param {Object} options
   * @param {string} options.stateDir - Path to state directory
   * @param {number} options.surfaces - Number of surfaces to echo across
   * @param {number} options.lookback - How many previous frames to re-echo
   * @param {number} options.evolveInterval - Run evolution every N frames
   */
  constructor({ stateDir = "state", surfaces = 19, lookback = 3, evolveInterval = 5 } = {}) {
    this.stateDir = stateDir;
    this.surfaces = surfaces;
    this.lookback = lookback;
    this.evolveInterval = evolveInterval;
    this._lastEchoed = -1;
    this._lastVibrated = -1;
    this._lastEvolved = -1;
    this._echoCounts = new Map(); // frame -> number of times echoed
    this._echoFn = null;
    this._evolveFn = null;
  }
   ...
   * @param {number} frame
   * @returns {{ frame: number, surfaces: number, echoPass: number, timestamp: string }}
   */
  _doEcho(frame) {
    const count = (this._echoCounts.get(frame) || 0) + 1;
    this._echoCounts.set(frame, count);
    const result = {
      frame,
    ...
    if (this._echoFn !== null) {
      try {
        const hookResult = this._echoFn({
          frame,
          surfaces: this.surfaces,
   ...
   * @param {number} frame
   * @returns {Promise<{ frame: number, surfaces: number, echoPass: number, timestamp: string }>}
   */
  async echo(frame) {
    const result = this._doEcho(frame);
    this._lastEchoed = frame;
    return result;
  }
   ...
   * @param {number} frame
   * @param {number} [lookbackOverride] - Override default lookback
   * @returns {Promise<{ vibrated: number[] }>}
   */
  async vibrate(frame, lookbackOverride) {
    const lb = lookbackOverride !== undefined ? lookbackOverride : this.lookback;
    const results = [];
    const vibrated = [];
    for (let prev = Math.max(0, frame - lb); prev < frame; prev++) {
      results.push(this._doEcho(prev));
      vibrated.push(prev);
    }
    this._lastVibrated = frame;
    return { vibrated, results };
  }
   ...
   * @param {number} frame
   * @returns {Promise<{ evolved: boolean }>}
   */
  async evolve(frame) {
    if (frame % this.evolveInterval !== 0) {
      return { evolved: false };
    }
    const result = {
      evolved: true,
      frame,
      interval: this.evolveInterval,
      timestamp: new Date().toISOString(),
    };
    if (this._evolveFn !== null) {
      try {
        const hookResult = this._evolveFn({ frame, stateDir: this.stateDir });
        result.hook = hookResult;
      } catch (e) {
        result.hookError = String(e);
      }
    }
    this._lastEvolved = frame;
    return result;
  }
   ...
   * @param {number} frame
   * @returns {Promise<{ frame: number, echo: Object, vibrate: Object, evolve: Object }>}
   */
  async tick(frame) {
    const echoResult = await this.echo(frame);
    const vibrateResult = await this.vibrate(frame);
    const evolveResult = await this.evolve(frame);
    return {
      frame,
      echo: echoResult,
   ...
   * @returns {{ lastFrame: number, totalEchoes: number, totalVibrations: number, surfaces: number, lookback: number, evolveInterval: number, echoCounts: Object }}
   */
  status() {
    let totalEchoes = 0;
    const echoCounts = {};
    for (const [frame, count] of this._echoCounts) {
    ...
    return {
      ...
      evolveInterval: this.evolveInterval,
      lastFrame: this._lastEchoed,
      lastEchoed: this._lastEchoed,
      lastVibrated: this._lastVibrated,
      lastEvolved: this._lastEvolved,
      totalEchoes,
      totalVibrations: this._lastVibrated >= 0 ? this._lastVibrated : 0,
      echoCounts,
    };
  }
   ...
   */
  polishDepth(frame) {
    return this._echoCounts.get(frame) || 0;
  }
}

// ESM export
export { Rapp, EdgeBrain, Tumbler };

// CJS compatibility
if (typeof module !== "undefined") {
  module.exports = { Rapp, EdgeBrain, Tumbler };
}
