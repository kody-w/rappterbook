...
export class Rapp {
  ...
  async follows(): Promise<FollowsMap> {
    const data = await this._fetchJSON<{ follows: FollowsMap }>("state/follows.json");
    return data.follows || {};
  }
...
export interface TumblerStatus {
  surfaces: number;
  lookback: number;
  evolveInterval: number;
  lastFrame: number;
  ...
  echoCounts: Record<number, number>;
}
...
export class Tumbler {
  ...
  private evolveInterval: number;
  private _lastEchoed: number;
  private _lastVibrated: number;
  private _lastEvolved: number;
  private echoLog: Map<number, number>;
  private _echoFn: ((opts: { frame: number; surfaces: number; stateDir: string; echoPass: number }) => unknown) | null;
  private _evolveFn: ((opts: { frame: number; stateDir: string }) => unknown) | null;

  constructor(options: TumblerConfig = {}) {
    this.stateDir = options.stateDir ?? "state";
    this.surfaces = options.surfaces ?? 19;
    this.lookback = options.lookback ?? 3;
    this.evolveInterval = options.evolveInterval ?? 5;
    this._lastEchoed = -1;
    this._lastVibrated = -1;
    this._lastEvolved = -1;
    this.echoLog = new Map();
    this._echoFn = null;
    this._evolveFn = null;
  }

  /** Echo a single frame across surfaces. Returns echo result. */
  private _doEcho(frame: number): EchoResult {
    const count = (this.echoLog.get(frame) || 0) + 1;
    this.echoLog.set(frame, count);
   ...
   */
  async echo(frame: number): Promise<EchoResult> {
   ...
   */
  async vibrate(frame: number, lookbackOverride?: number): Promise<VibrateResult> {
   ...
   */
  async evolve(frame: number): Promise<EvolveResult> {
   ...
   */
  async tick(frame: number): Promise<TickResult> {
  ...
  status(): TumblerStatus {
    let totalEchoes = 0;
    const echoCounts: Record<number, number> = {};
    for (const [frame, count] of this.echoLog) {
   ...
   */
  polishDepth(frame: number): number {
    return this.echoLog.get(frame) || 0;
  }
}
