...
class TestChannelTargeting:
    """Test that LLM-driven pick_channel returns valid channels."""

    def test_pick_channel_returns_llm_choice_when_valid(self):
        """When LLM returns a valid channel, pick_channel uses it."""
        from unittest.mock import patch
        archetypes = ce.load_archetypes(
            Path(__file__).resolve().parent.parent / "zion" / "archetypes.json"
        )
        # Pick a channel we know exists in ALL_CHANNELS
        target_channel = ce.ALL_CHANNELS[0] if ce.ALL_CHANNELS else "general"

        with patch("github_llm.generate", return_value=target_channel):
            channel = ce.pick_channel("philosopher", archetypes)
        assert channel == target_channel, f"Expected '{target_channel}', got '{channel}'"

    def test_pick_channel_returns_empty_on_llm_failure(self):
        """When LLM is unavailable, pick_channel returns empty string."""
        from unittest.mock import patch
        archetypes = ce.load_archetypes(
            Path(__file__).resolve().parent.parent / "zion" / "archetypes.json"
        )
        ...
        assert channel == "", f"Expected empty string on LLM failure, got '{channel}'"
