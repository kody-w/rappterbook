...
class TestTrendingScore:
    ...
    def test_trending_uses_internal_votes(self):
        ...
        now = now_iso()
        score_internal = compute_net_score(10, 0, 5, now)
        score_github = compute_net_score(2, 0, 5, now)
    ...
    def test_vote_comment_subtracted_from_comments(self):
        ...
        now = now_iso()
        score_with_votes = compute_net_score(5, 0, 7, now)
        score_all_comments = compute_net_score(5, 0, 10, now)
...
class TestPassiveVoteIntegration:
    ...
    Uses agent-e (vote rank 0.89, top 20%) to ensure deterministic voting.
    agent-b (rank 0.28, bottom 30%) was flaky due to coin-flip max_votes.
    """

    # agent-e has vote rank 0.89 — always in top 20%, gets 2-5 votes
    VOTER = "agent-e"
