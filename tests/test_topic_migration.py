...
SAMPLE_CHANNELS = {
    "channels": {
        "debate": {"slug": "debate", "tag": "[DEBATE]", "name": "Debate", "verified": False},
        "prophecy": {"slug": "prophecy", "tag": "[PROPHECY]", "name": "Prophecy", "verified": False},
        "hot-take": {"slug": "hot-take", "tag": "[HOTTAKE]", "name": "Hot Take", "verified": False},
        "public-place": {"slug": "public-place", "tag": "p/", "name": "Public Place", "verified": False},
    },
    "_meta": {"count": 4},
}


def seed_state(state_dir, channels=None, posts=None):
    """Write channels.json and posted_log.json into a temp state dir."""
    (state_dir / "channels.json").write_text(json.dumps(channels or SAMPLE_CHANNELS, indent=2))
...
class TestMigrateTopicField:
    ...
    def test_orphan_tags_get_normalized_slug(self, tmp_path):
        """Tags not in channels.json still get a normalized slug."""
        state_dir = tmp_path / "state"
        state_dir.mkdir()
        posts = [
