...
class TestAutonomyVoteAction:
    ...
    def test_vote_action_calls_api(self, mock_comment, mock_pace, tmp_state):
        ...
        # Patch STATE_DIR so _has_already_voted reads from tmp_state
        with patch.object(zion_autonomy, "STATE_DIR", tmp_state):
            execute_action(
...
class TestExecuteThread:
    ...
    @patch("zion_autonomy.pick_discussion_to_comment")
    def test_execute_thread_dry_run(self, mock_pick, tmp_state):
        ...
        mock_pick.return_value = discussions[0]
    ...
    def test_execute_thread_api_calls(self, mock_gen, mock_comment, mock_reply, mock_pick, tmp_state):
    ...
    @patch("zion_autonomy.pick_discussion_to_comment")
    @patch("zion_autonomy.add_discussion_comment")
    def test_execute_thread_mid_failure(self, mock_comment, mock_pick, tmp_state):
    ...
    @patch("zion_autonomy.add_discussion_comment")
    @patch("zion_autonomy.generate_comment")
    def test_execute_thread_updates_state(self, mock_gen, mock_comment, mock_reply, mock_pick, tmp_state):
...
class TestCommentFailureHandling:
    ...
    @patch("zion_autonomy.generate_comment")
    @patch("zion_autonomy.pick_discussion_to_comment")
    def test_comment_none_skips(self, mock_pick, mock_gen, tmp_state):
        """When generate_comment returns None, post is skipped with [fail] log."""
        from zion_autonomy import _execute_comment

        # Ensure pick returns a valid target so we reach generate_comment
    ...
    @patch("zion_autonomy.generate_comment")
    @patch("zion_autonomy.pick_discussion_to_comment")
    def test_comment_exception_skips(self, mock_pick, mock_gen, tmp_state):
...
class TestRenameChangeEntryField:
    def test_rename_feature_removed(self):
        """Verify rename feature was removed from zion_autonomy.py."""
        src = ROOT / "scripts" / "zion_autonomy.py"
        text = src.read_text()
        assert "def _execute_rename" not in text, "Dead _execute_rename should be removed"
