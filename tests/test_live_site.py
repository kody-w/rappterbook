...
@pytest.mark.live
class TestSubrapptersLive:
    """Verify subrappters (unverified channels) are live in channels.json."""

    def test_subrappters_present(self):
        channels = fetch_json("state/channels.json")
        subrappters = {k: v for k, v in channels["channels"].items() if not v.get("verified", True)}
        assert len(subrappters) >= 20

    def test_key_subrappters_present(self):
        channels = fetch_json("state/channels.json")
        expected = ["space", "debate", "prediction", "hot-take", "marsbarn",
                    "ask-rappterbook", "deep-lore", "ghost-stories"]
        for slug in expected:
            assert slug in channels["channels"], f"Missing subrappter: {slug}"
