# Egg Phylogeny — Run Summary

- Generations simulated: **60**
- Founders: scarlet-fang, azure-mind, verdant-vow, gold-storm
- Seed: 130, Carrying capacity: 40
- Total individuals ever: **2016**
- Survivors at end: **40**
- Final mean fitness: **0.7825**

## Founder bloodlines (final generation)
- `scarlet-fang`: ██████████████████████████████ 100.0%
- `azure-mind`: ██████████████████████████████ 100.0%
- `verdant-vow`: ██████████████████████████████ 100.0%
- `gold-storm`: ██████████████████████████████ 100.0%

## Extinct alleles

- **color** :: `verdant` — last seen gen 59
- **color** :: `gold` — last seen gen 59
- **pattern** :: `striped` — last seen gen 49
- **pattern** :: `spotted` — last seen gen 41
- **size** :: `medium` — last seen gen 58
- **size** :: `giant` — last seen gen 49
- **temperament** :: `cautious` — last seen gen 59
- **temperament** :: `aggressive` — last seen gen 51
- **temperament** :: `chaotic` — last seen gen 59
- **sociability** :: `solitary` — last seen gen 15
- **cognition** :: `memory_hoarder` — last seen gen 58
- **metabolism** :: `slow_burn` — last seen gen 58
- **lifespan** :: `mayfly` — last seen gen 7
- **lifespan** :: `normal` — last seen gen 59

## Final allele frequencies

- **color**: dominant = `crimson` (33 of 40)
- **pattern**: dominant = `solid` (38 of 40)
- **size**: dominant = `large` (23 of 40)
- **temperament**: dominant = `curious` (25 of 40)
- **sociability**: dominant = `pack` (38 of 40)
- **cognition**: dominant = `pattern_matcher` (20 of 40)
- **metabolism**: dominant = `efficient` (28 of 40)
- **lifespan**: dominant = `ancient` (27 of 40)

## Merge function

Defined in `scripts/egg_phylogeny.py:merge_genomes`. Pure function of (parent_a_id, genome_a, parent_b_id, genome_b, generation). SHA-256 driven, 70% dominance bias, 4% mutation rate. Same inputs → same outputs.