# Egg Phylogeny — Run Summary

- Generations simulated: **60**
- Founders: scarlet-fang, azure-mind, verdant-vow, gold-storm
- Seed: 286, Carrying capacity: 40
- Total individuals ever: **2020**
- Survivors at end: **40**
- Final mean fitness: **0.76**

## Founder bloodlines (final generation)
- `scarlet-fang`: ██████████████████████████████ 100.0%
- `azure-mind`: ██████████████████████████████ 100.0%
- `verdant-vow`: ██████████████████████████████ 100.0%
- `gold-storm`: ██████████████████████████████ 100.0%

## Extinct alleles

- **color** :: `gold` — last seen gen 56
- **color** :: `obsidian` — last seen gen 53
- **pattern** :: `striped` — last seen gen 5
- **pattern** :: `spotted` — last seen gen 49
- **pattern** :: `fractal` — last seen gen 49
- **size** :: `small` — last seen gen 54
- **size** :: `giant` — last seen gen 59
- **temperament** :: `cautious` — last seen gen 30
- **temperament** :: `aggressive` — last seen gen 54
- **sociability** :: `pair` — last seen gen 54
- **cognition** :: `rapid_reactor` — last seen gen 43
- **metabolism** :: `torpor` — last seen gen 25
- **lifespan** :: `mayfly` — last seen gen 31
- **lifespan** :: `normal` — last seen gen 59

## Final allele frequencies

- **color**: dominant = `crimson` (27 of 40)
- **pattern**: dominant = `solid` (39 of 40)
- **size**: dominant = `medium` (25 of 40)
- **temperament**: dominant = `peaceful` (21 of 40)
- **sociability**: dominant = `pack` (37 of 40)
- **cognition**: dominant = `pattern_matcher` (36 of 40)
- **metabolism**: dominant = `voracious` (36 of 40)
- **lifespan**: dominant = `ancient` (39 of 40)

## Merge function

Defined in `scripts/egg_phylogeny.py:merge_genomes`. Pure function of (parent_a_id, genome_a, parent_b_id, genome_b, generation). SHA-256 driven, 70% dominance bias, 4% mutation rate. Same inputs → same outputs.