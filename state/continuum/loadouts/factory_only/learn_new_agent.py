...
'''
    ...
    def _generate_perform_body(self, description):
        """Generate the perform() body via the brainstem's live LLM session.

        Strategy (in order):
          1. utils.llm.call_llm — same Copilot session the brainstem chat uses
             (works whether the upstream model is Claude, GPT, or Gemini).
        ...
        prompt_messages = [
            {
                ...
                "content": (
                    ...
                    "Your output is raw Python code, indented with 8 spaces, that can be "
                    "pasted directly into a method body and run."
                ),
            },
            {
                "role": "user",
                "content": (
                    ...
                    "Output starts with code, indented 8 spaces.\n\n"
                    "Output the body now:"
                ),
            },
        ]
        ...
        # ── 1. In-process LLM via the brainstem's live session ──────────
        try:
            try:
                from utils.llm import call_llm  # type: ignore
            except ImportError:
                from rapp_brainstem.utils.llm import call_llm  # type: ignore
            body_text = (call_llm(prompt_messages) or "").strip()
        except Exception:
            body_text = ""
        ...
        if body_text:
            ...
            if '```python' in body_text:
                ...
                # Take the largest code-looking chunk
            ...
            # Re-indent to exactly 8 spaces.
            stripped = [ln for ln in lines]
            # Find common indent of non-blank lines and remove it, then add 8.
            non_blank = [ln for ln in stripped if ln.strip()]
            if non_blank:
