...
class SwarmFactoryAgent(BasicAgent):
    ...
    def perform(self, action="build", swarm_name="MySwarm", description="", exclude="",
        ...
        for path in agent_files:
            ...
            try:
                ...
                if cls_name in skip or cls_name.replace("Agent", "") in skip:
                    continue
                # Extract the agent's metadata.parameters block from inside
                # the class body so the generated singleton can carry the
                # real tool schema instead of {"type":"object","properties":{}}.
                # Walks: __init__ → self.metadata = {...} → "parameters": {...}
                params_dict = None
                for cls_node in classes:
                    ...
                    for body_node in cls_node.body:
                        ...
                        for stmt in body_node.body:
                            ...
                            if is_self_metadata and isinstance(stmt.value, ast.Dict):
                                for k, v in zip(stmt.value.keys, stmt.value.values):
                                    if isinstance(k, ast.Constant) and k.value == "parameters":
                                        try:
                                            params_dict = ast.literal_eval(v)
                ...
                # Don't swallow silently — but also don't crash the build.
                # The factory should converge what it can and tell the
                # caller what it skipped.
                print(f"[swarm_factory] skipped {fname}: {type(exc).__name__}: {exc}")
                continue
        ...
        # Real fix: pull params from the top agent's actual metadata.parameters
        # (extracted via AST in the source-walk above). The previous code
        # called top_info.get("metadata") which never existed in the sources
        # dict, so every generated singleton shipped {"properties":{}}. With
        # an empty params block the calling LLM cannot supply ANY arguments
        # to the swarm — the tool-call machinery would fail validation. This
        # was the silent killer.
        top_params = top_info.get("params") or {"type": "object", "properties": {}, "required": []}
        out += f'            "parameters": {json.dumps(top_params)}\n'
        out += f'        }}\n'
        ...
        # Write output INTO agents/ so the brainstem hot-loads it on the
        # next request. The previous code wrote to os.path.dirname(agents_dir)
        # — the brainstem source root — where the agent loader does NOT look.
        # Result was a generated singleton sitting one level up, invisible to
        # the discovery glob. Writing into agents_dir makes "build" actually
        # produce a runnable swarm without manual relocation.
        output_fname = f"{slug}_agent.py"
        output_path = os.path.join(agents_dir, output_fname)
        if os.path.exists(output_path):
            return json.dumps({
                "status": "error",
                "message": (
                    f"agents/{output_fname} already exists. Refusing to overwrite. "
                    f"Either pick a different swarm_name, or call "
                    f"SwarmFactory(action='uninstall', swarm_name='{public_name}') first."
                ),
            })
