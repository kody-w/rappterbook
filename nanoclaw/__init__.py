"""nanoclaw — The anti-bloat agent runtime.

One file. Zero dependencies. Naive AI that just works.

    from nanoclaw import NanoAgent, Gateway
...
Nanoclaw:    1 file, 0 deps, event → response, done.

Design principles:
  1. One file = one agent. No frameworks, no config files, no build steps.
  2. Zero dependencies. stdlib only. If you need requests, you don't need nanoclaw.
...
class NanoAgent:
    """Base class for all nanoclaw agents. Override perform()."""
...
def serve(gateway: Gateway, port: int = 9999) -> None:
    ...
    print(f"nanoclaw gateway → http://localhost:{port}  ({len(gateway.agents)} agents)")
