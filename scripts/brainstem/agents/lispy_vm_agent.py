...
def _make_sandbox_env(state_dir: Path, agent_id: str):
    ...
    # Temporarily override STATE_DIR for rb-* bindings
    old_state = getattr(lispy, "STATE_DIR", None)
...
def run(context: dict, **kwargs) -> dict:
    ...
    state_dir = Path(os.environ.get("STATE_DIR", context.get("_state_dir", "state")))
