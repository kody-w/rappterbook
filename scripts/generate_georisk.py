...
"""Generate static simulation data for the GeoRisk Dashboard.

Runs the solar system simulation for N steps and writes the event stream
to docs/georisk/sim-data.json. The frontend replays these events with
timing to create the illusion of a live feed.
...
# ---------------------------------------------------------------------------
# Planet / colony definitions (ported from data_engine.py)
# ---------------------------------------------------------------------------
...
def get_initial_state() -> Dict[str, Any]:
    """Build the initial simulation state for all planets."""
    state: Dict[str, Any] = {}
    for body_id, data in BODIES.items():
        state[body_id] = {
            "name": data["name"],
            "colonies": {
                c["id"]: {"name": c["name"], "lat": c["lat"], "lng": c["lng"], "health": random.randint(70, 100)}
...
def generate_event(state: Dict[str, Any]) -> Dict[str, Any]:
    """Generate a single simulation event, mutating state in place."""
    body_id = random.choice(list(BODIES.keys()))
    body_data = BODIES[body_id]

    event_type = random.choices(
        ["health", "resource", "news", "bot_radar"],
        weights=[20, 30, 40, 10],
    )[0]

    if event_type == "health":
        colony = random.choice(body_data["colonies"])
        c_id = colony["id"]
        ...
        return {"type": "health", "body_id": body_id, "colony_id": c_id, "health": new_health}

    elif event_type == "resource":
        res = random.choice(list(state[body_id]["resources"].keys()))
        if "%" in res:
            change = random.uniform(-1.5, 2.0)
            state[body_id]["resources"][res] = round(max(0.0, min(100.0, state[body_id]["resources"][res] + change)), 2)
        else:
            change = random.uniform(-5.0, 7.5)
            state[body_id]["resources"][res] = round(max(0.0, state[body_id]["resources"][res] + change), 2)
        return {"type": "resource", "body_id": body_id, "res_name": res, "value": state[body_id]["resources"][res]}

    elif event_type == "news":
        colony = random.choice(body_data["colonies"])
        c_id = colony["id"]
        headline = random.choice(body_data["event_templates"]).format(colony["name"])
        ...
        return {"type": "news", "body_id": body_id, "headline": headline, "colony_id": c_id}

    else:  # bot_radar
        colony = random.choice(body_data["colonies"])
        lat = round(max(-90, min(90, colony["lat"] + random.uniform(-10.0, 10.0))), 2)
        lng = round(max(-180, min(180, colony["lng"] + random.uniform(-10.0, 10.0))), 2)
        return {
            "type": "bot_radar",
            "body_id": body_id,
            "bot_id": f"{body_id[:3].upper()}-{random.randint(100, 999)}",
            "colony": colony["name"],
            "lat": lat,
            "lng": lng,
        }
...
def generate_simulation(n_events: int = 500, seed: int = 42) -> Dict[str, Any]:
    """Run the full simulation and return init state + event stream."""
    random.seed(seed)
    state = get_initial_state()
    events: List[Dict[str, Any]] = []

    for _ in range(n_events):
        events.append(generate_event(state))
...
def main() -> None:
    """Generate sim-data.json for the static GeoRisk Dashboard."""
    repo_root = Path(__file__).resolve().parent.parent
    out_dir = repo_root / "docs" / "georisk"
    out_dir.mkdir(parents=True, exist_ok=True)

    n_events = int(sys.argv[1]) if len(sys.argv) > 1 else 500
    seed = random.randint(1, 999999)

    sim = generate_simulation(n_events=n_events, seed=seed)
    out_path = out_dir / "sim-data.json"
    with open(out_path, "w") as f:
        json.dump(sim, f, separators=(",", ":"))

    size_kb = out_path.stat().st_size / 1024
    print(f"Generated {n_events} events (seed={seed}) → {out_path} ({size_kb:.1f} KB)")
