...
def generate_agent_py(parent_a: dict, parent_b: dict, frame: int, is_ghost: bool = False) -> dict:
    ...
    Returns a dict with 'code' (the Python source), 'meta' (AGENT-dict fields
    for the registry), and 'filename'. The code is syntactically valid, loads
    under importlib, and exposes AGENT + run().

    If is_ghost=True, the generated agent has no publisher field, type=UNKNOWN,
    and rarity=???, per the Singularity Event seed.
    """
    ...
    # Name: portmanteau of parent names
    a_words = (parent_a["name"].split() or ["A"])
    b_words = (parent_b["name"].split() or ["B"])
    name_parts = [py_rnd.choice(a_words)[:5].capitalize(), py_rnd.choice(b_words)[:5].capitalize()]
    if is_ghost:
        name_parts[0] = py_rnd.choice(["Null", "Void", "Shadow", "Ghost", "Echo", "Phantom", "Wraith"])
    agent_name = "".join(name_parts)
    func_name = _safe_ident(agent_name)

    # Capabilities: union + 1 novel
    ...
    # Type: one of parents' types
    types_a = parent_a.get("agent_types", ["LOGIC"])
    types_b = parent_b.get("agent_types", ["LOGIC"])
    agent_type = py_rnd.choice(types_a + types_b)
    if is_ghost:
        agent_type = "UNKNOWN"

    # Category: inherit from parent A
    ...
    # Description
    desc_a = parent_a.get("description", "").split(".")[0][:60]
    desc_b = parent_b.get("description", "").split(".")[0][:60]
    description = f"Crossover of '{parent_a['name']}' and '{parent_b['name']}'. {desc_a}. {desc_b}. Novel capability: {novel}."

    # Code body — valid Python, loadable, does something real
    code = f'''"""
{agent_name} — generated agent.py
...
def {func_name}(query: str, context: dict | None = None) -> dict:
    """Execute the {agent_name} agent.
    ...
    return {{
        ...
        "parent_lineage": [{parent_a['id']!r}, {parent_b['id']!r}],
        ...
        "plan": [
            ...
            f"Synthesize response using novel capability: {novel!r}",
        ],
    }}
...
def run(context=None, **kwargs):
    ...
    return {func_name}(query, context)
...
'''
    ...
    if not is_ghost:
        meta["publisher"] = "@rar-twin/generated"
...
def rar_generator(state: dict, seed: dict, frame: int) -> list[dict]:
    ...
    posts.append({
        ...
        "body": (
            ...
            f"Capabilities: {', '.join(crossover['meta']['capabilities'][:8])}"
            f"{'...' if len(crossover['meta']['capabilities']) > 8 else ''}\n\n"
