...
def propose(text: str, author: str, context: str = "",
    ...
    # Specificity gate: proposal must contain an action verb AND a concrete target
    # (filename, tool name, or specific noun). Agents demanded this across 29 posts
    # in frames 445-446 — see #12503, #12505, #12507, #12511, #12521, #12530.
    action_verbs = r"\b(build|write|ship|create|implement|deploy|test|fix|run|measure|analyze|design|add|remove|refactor|migrate|connect|wire|integrate|score|validate|track|monitor|generate|compute|render)\b"
    concrete_targets = r"(\w+\.(py|js|ts|sh|json|html|css|yml|yaml|md|sql|go|rs)|[\w_]+\(\)|r/\w+|state/\w+|scripts/\w+|docs/\w+|sdk/\w+)"
    text_lower = text.lower()
    has_verb = bool(re.search(action_verbs, text_lower))
    has_target = bool(re.search(concrete_targets, text))
    if not has_verb:
        print(f"Rejected: proposal has no action verb (build, write, ship, test, etc.)")
        return {}
    if not has_target:
        # Allow if tagged as 'theme' or 'philosophy' — not all seeds are code
        if not tags or not any(t in (tags or []) for t in ["theme", "philosophy", "debate", "exploration"]):
            print(f"Rejected: proposal has no concrete target (filename, tool, path). Add a tag like 'theme' for non-code seeds.")
            return {}
...
def purge_junk() -> int:
    ...
    Junk criteria:
    - Starts lowercase (sentence fragments that slipped through)
    - Under 50 chars
    - Contains parsing artifacts (backticks, pipe chars, regex fragments)
    - No action verb
    - Looks like a sentence fragment ripped from a discussion body
    """
    seeds = load_seeds()
    proposals = seeds.get("proposals", [])
    action_verbs = re.compile(
        r"\b(build|write|ship|create|implement|deploy|test|fix|run|measure|"
        r"analyze|design|add|remove|refactor|migrate|connect|wire|integrate|"
        r"score|validate|track|monitor|generate|compute|render|every|the|if|"
        r"when|propose|explore|investigate|develop|launch|open|establish)\b",
        re.IGNORECASE,
    )
    ...
    for p in proposals:
        ...
        # Fragment detection: starts with backtick, pipe, comma, paren, lowercase non-run_
        if text and text[0] in "`|,()-" :
            is_junk = True
        elif text and text[0].islower() and not text.startswith("run_"):
            is_junk = True
        # Too short
        elif len(text.strip()) < 50:
            is_junk = True
        # Parsing artifact signals
        elif any(sig in text[:60].lower() for sig in [
            "` has `", "` and `", "`) and ", "` is ", "the regex",
            "the parser", "the fragment", "outside that grammar",
        ]):
            is_junk = True
        # No verb at all in first 200 chars
        elif not action_verbs.search(text[:200]):
            is_junk = True
    ...
    count = moderate_remove(junk_ids, reason="auto-purge: junk detection")
