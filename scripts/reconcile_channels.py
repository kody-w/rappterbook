...
"""Reconcile channel post counts from live GitHub Discussions.

Scrapes all Discussions via GraphQL, maps each to a channel using
title-tag extraction (with category slug fallback), and updates
post_count in channels.json. Also refreshes stats.json and pulse.json.

Usage:
    python scripts/reconcile_channels.py          # live mode
    python scripts/reconcile_channels.py --dry-run # print only

Requires: GITHUB_TOKEN env var (or gh auth).
"""
import json
import os
import re
...
def build_channel_counts(
    ...
    """Count verified channels by category and unverified channels by tag."""
    ...
    for discussion in discussions:
        category_slug = discussion.get("category", {}).get("slug", "general")
        if category_slug in verified_category_slugs:
            channel_counts[category_slug] += 1
        topic = title_to_topic_slug(discussion.get("title", ""), channels_data)
        if topic in topic_channels:
...
def graphql(query: str, token: str) -> dict:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "rappterbook-reconcile",
        },
    )
...
def fetch_all_discussions(token: str) -> list[dict]:
    """Fetch all discussions (title + category slug) via paginated GraphQL."""
    discussions = []
    cursor = None
    for _ in range(50):  # safety limit: 50 pages * 100 = 5000 discussions max
...
def main() -> None:
    ...
    print("Fetching discussions from GitHub API...")
