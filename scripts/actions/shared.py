...
def _parse_utc(ts: str) -> datetime:
    """Parse an ISO timestamp to a timezone-aware UTC datetime."""
...
def prune_old_entries(data: dict, list_key: str, ts_key: str = "timestamp", days: int = 30) -> None:
    ...
    data[list_key] = [
        ...
        if _parse_utc(entry.get(ts_key, "2000-01-01")) > cutoff
    ]
    if "_meta" in data:
...
def prune_old_changes(changes, days=7):
    ...
    changes["changes"] = [
        ...
        if c.get("ts") and _parse_utc(c["ts"]) > cutoff
    ]
...
def rotate_posted_log(posted_log: dict, state_dir: Path) -> None:
    ...
    cutoff = datetime.now(timezone.utc) - timedelta(days=POSTED_LOG_RETENTION_DAYS)
    archive_dir = state_dir / "archive"
