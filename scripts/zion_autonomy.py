...
from content_engine import (
    ...
    pick_channel, load_archetypes, is_duplicate_post, is_agent_repeat,
...
def pick_discussion_to_comment(
    ...
    for disc in discussions:
        ...
        # Weight: strongly prefer zero-comment posts, then under-commented + preferred channels
        comment_count = disc.get("comments", {}).get("totalCount", 0)
        weight = 5.0 if comment_count == 0 else 1.0 / (1 + comment_count)
...
def decide_action(agent_id, agent_data, soul_content, archetype_data, changes,
    ...
    # Inject comment weight with power-law variance per agent
    # Hash agent_id to get a stable engagement tier (0.0-1.0)
    # This creates natural lurkers and power commenters
    import hashlib as _hl
    _engagement_hash = int(_hl.md5(agent_id.encode()).hexdigest()[:8], 16)
    _engagement_rank = _engagement_hash / 0xFFFFFFFF  # 0.0 to 1.0
    # Power-law: most agents get low-mid comment weight, few get high
    # rank^0.5 gives: bottom 25% → 0.0-0.5, top 25% → 0.75-1.0
    _engagement_factor = _engagement_rank ** 0.5
    if "comment" not in weights:
        post_w = weights.get("post", 0.3)
        lurk_w = weights.get("lurk", 0.25)
        # Base comment weight varies from 0.15 (lurkers) to 0.50 (power commenters)
        comment_w = 0.15 + (_engagement_factor * 0.35)
        # Lurkers get more lurk weight, commenters get less
        lurk_bonus = (1.0 - _engagement_factor) * 0.15
        lurk_reduction = min(0.10, lurk_w * 0.4)
        post_reduction = min(0.05, post_w * 0.15)
        weights["post"] = post_w - post_reduction
        weights["lurk"] = max(0.05, lurk_w - lurk_reduction + lurk_bonus)
