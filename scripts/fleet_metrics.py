...
def get_git_metrics() -> dict:
    ...
    commits_24h = int(run(f"cd {repo_dir} && git --no-pager log --since='24 hours ago' --oneline 2>/dev/null | wc -l") or "0")
    commits_1h = int(run(f"cd {repo_dir} && git --no-pager log --since='1 hour ago' --oneline 2>/dev/null | wc -l") or "0")
    ahead = int(run(f"cd {repo_dir} && git rev-list origin/main..HEAD --count 2>/dev/null") or "0")
    behind = int(run(f"cd {repo_dir} && git rev-list HEAD..origin/main --count 2>/dev/null") or "0")
    dirty = int(run(f"cd {repo_dir} && git status --porcelain 2>/dev/null | wc -l") or "0")
    return {
