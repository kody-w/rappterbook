...
from state_io import load_json, now_iso, verify_consistency, reconcile_counts
...
def main() -> None:
    ...
    # Auto-fix drift instead of just logging it
    if issues:
        fixes = reconcile_counts(STATE_DIR)
        state_consistency["auto_fixed"] = fixes
    ...
    print(f"Autonomy log: {r['agents_activated']} agents, "
          ...
          f"{r['votes']} votes, {r.get('lurks', 0)} lurks, "
          f"{r['failures']} failures, {r['skips']} skips")
